...
```
The average number of grades is close to 10 for every group, there is more dispersion in some groups.
...
Except for male in group 15, the average stays very close to 10, the dispersion varies depending on sex in some groups (1, 9, 16, 5, 17)
...
The positive linear relationship observed seems to weak to draw conclusions.
...
There is a clear negative relationship between average grades in Shamanism and Spirit Communication and Drumming and Music in group 17.
...
There are clear differences in the average grades of the different groups.
...
```
In most groups the average grades of male and female students is relatively close.
...
There are `r fail_but_good` students who do not pass despite having a final grade superior to 10.
...
```{r}
fail_by_course <- student_course_avg |>
  filter(course_avg < 5) |>
  summarise(
    nb_students_below_5 = n(),
    .by = course
  )


knitr::kable(fail_by_course)

```

Tribal Laws and Custom is the only course where students have averages below 5.
...
```{r}
age_success <- pass_table |>
  inner_join(students |> select(id, birth_date), by = "id") |>
  mutate(
    age = floor(time_length(today() - birth_date, unit = "year")) # I used floor to get a smoother graph
  ) |>
  summarise(
...
```
We cannot draw any correlation between age and pass rate
